...
__global__ void double_scalars(size_t n, scalar_t *out)
{
...
template __global__ void double_scalars(size_t n, scalar_t *out);
#endif
...
public:
                     ...
                     size_t ffi_affine_sz = sizeof(affine_t))
    {
        ...
        try
        {
            ...
            double_scalars<scalar_t><<<16, 16, 0, gpu[2]>>>(num, &d_scalars[0]);
            CUDA_OK(cudaGetLastError());
            ...
            for (uint32_t i = 0; i < batch; i++)
            {
                ...
                if (i < batch - 1)
                {
                    ...
                    gpu[2].wait(ev);
                    double_scalars<scalar_t><<<16, 16, 0, gpu[2]>>>(num, &d_scalars[scalars ? 0 : h_off]);
                    CUDA_OK(cudaGetLastError());
...
// template <typename scalar_t>
// RustError scalar_add_test(size_t n, const scalar_t *x, const scalar_t *y, scalar_t *out)
// {
//     try
//     {
//         scalar_t *d_x, *d_y, *d_out;

//         // Allocate Unified Memory – accessible from CPU or GPU
//         cudaMalloc(&d_x, n * sizeof(scalar_t));
//         cudaMalloc(&d_y, n * sizeof(scalar_t));
//         cudaMalloc(&d_out, n * sizeof(scalar_t));

//         cudaMemcpy(d_x, x, n * sizeof(scalar_t), cudaMemcpyHostToDevice);
//         cudaMemcpy(d_y, y, n * sizeof(scalar_t), cudaMemcpyHostToDevice);

//         // Run kernel on 1M elements on the GPU
//         scalar_add<<<1, 1>>>(n, d_x, d_y, d_out);
//         CUDA_OK(cudaGetLastError());

//         cudaMemcpy(out, d_out, n * sizeof(scalar_t), cudaMemcpyDeviceToHost);
